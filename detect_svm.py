...
if __name__ == '__main__':
  ...
  for u in users:
      ...
      #use multiple users for making the validation set
      v_train = []
      if validation == True:
          #print "Preparing Validation set"
          train_users =[u]
          while len(train_users) !=2: #gather trips from 10 other random users
            ...
            if false_user not in train_users:
              ...
              v_train.extend( generateFeatures(val_locs,num_trips=100) )
      ...
      ''' 
      for reg in [1, 10, 30, 70, 100, 1000, 1500]:
          g = 10.0/float((3*10))
          clf = svm.SVC(kernel='rbf', C=reg, gamma=g)
          scores = cross_validation.cross_val_score(clf, train, labels, cv=5 )
          print ("Cross Validation Kernel SVM(cv=5) scores: reg=%f mean=%f dev=%f" % (reg,scores.mean(),scores.std()) )
          clf_l = svm.SVC(kernel='linear', C=reg)
          scores = cross_validation.cross_val_score(clf_l, train, labels, cv=5 )
          print ("Cross Validation Linear SVM(cv=5) scores: reg=%f mean=%f dev=%f" % (reg,scores.mean(),scores.std()) )
      '''
      reg = 10
      g = 10.0/float((3*10))
      clf_f = svm.SVC(kernel='rbf', C=reg, gamma=g, probability=True)
      clf_f.fit(X_train,y_train)
      #print "Classifier:"
      #print clf, clf.get_params()
      print "Validation set score for best C=: " ,reg, roc_auc_score(y_val, clf_f.predict_proba(X_val)[:,1])
